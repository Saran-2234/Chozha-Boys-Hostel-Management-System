...
const Sidebar = ({ setActiveSection, activeSection, onLogout, isDarkMode, sidebarOpen, setSidebarOpen }) => {
  return (
    <aside className={`sidebar fixed left-0 top-0 h-full w-64 z-30 flex flex-col transition-transform duration-300 ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-gray-100 text-gray-900'} shadow-lg ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
